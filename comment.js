    ...
    <div class=bd>
        <div class="card submit">
            <form onsubmit="return false" id="create_post">
                <ul>
                    <li><span>Name</span><input class=line name=author_name required placeholder="Name" value="${author_name}">
                        <li><span>Email</span><input class=line name=author_email type=email required placeholder="邮箱" value="${author_email}">
                            <li><span>内容：</span><textarea class="line" name="message" required placeholder="由于 disqus 的限制，评论内容需要审核后才会出现"></textarea>
                                <li><input type=hidden name=parent value="${parentId}">
                                    ...
                                    <li><a href="#close" onclick="window.close();void(0)">Cancel</a></ul>
            </form>
        </div>
    </div>
    <footer></footer>
  ...
  buildComment: function(comment) {
    ...
    <div class="comment-msg">
      ...
      <span class="author" data-name="${comment.author.name}"> <a href="${comment.author.profileUrl}">${comment.author.name}</a> </span>
        ...
        <span class="actions">
          <a class="reply" href="javascript: postComment('${comment.id}')">Reply</a>
        </span>
      </div>
      <div class="text">
        ${comment.message}
...
function CommentAPI(forum, apiPath, selector, ident) {
  ...
  var root = $(`<div class="comments" id="cmt_root">
      <div class="comment-header clearfix">
          <div class="comment-title">Comments&nbsp;(<span id="comment_count">0</span>)</div>
          <a href="javascript:postComment()" class="post-comment-btn btn">Comment</a></div>
          </div>`);
